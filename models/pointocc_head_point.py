...
class PointOccHeadPoint(BaseModule):
    def __init__(self,
                 num_classes,
                 in_channels,
                 num_query,
    ...
    def _init_layers(self):
        self.init_points_ctr = nn.Embedding(self.num_query, 3)
        self.init_points_offset = nn.Embedding(1, 3 * self.num_refines)
        nn.init.uniform_(self.init_points_ctr.weight, 0, 1)
    ...
    def forward(self, mlvl_feats, img_metas):
        B, Q, R = mlvl_feats[0].shape[0], self.num_query, self.num_refines
        centers = self.init_points_ctr.weight[None, ...].repeat(B, 1, 1)
        offsets = self.init_points_offset.weight[None, ...].repeat(B, 1, 1)

        offsets = offsets.reshape(B, 1, R, 3)
        query_points = centers.unsqueeze(2) + offsets
        query_feat = query_points.new_zeros(B, Q, self.embed_dims)
