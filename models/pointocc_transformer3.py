...
class PointOccTransformer3(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_points=4,
        ...
        super(PointOccTransformer3, self).__init__(init_cfg=init_cfg)
...
class PointOccTransformerDecoder(BaseModule):
        ...
        for i in range(num_layers):
            self.decoder_layers.append(
                PointOccTransformerDecoderLayer(
                    ...
                    num_refines, layer_idx=i, offset_scale=offset_scale,
    ...
    def forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        query_points = query_points.detach().unsqueeze(2)
        query_points = query_points.expand(-1, -1, self.num_refines, -1)
...
class PointOccTransformerDecoderLayer(BaseModule):
    def __init__(self,
        ...
        self.position_encoder = nn.Sequential(
            nn.Linear(3 * num_refines, self.embed_dims), 
        ...
        self.sampling = PointOccSampling(embed_dims, num_frames=num_frames, num_groups=4,
                                         ...
                                         num_refines=num_refines, pc_range=pc_range)
    ...
    def forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        query_feat = self.norm1(self.self_attn(query_points, query_feat))
        ...
        reg_offset = self.reg_branch(query_feat) * self.offset_scale  # [B, Q, 3P]
        refine_pt = self.refine_points(query_points.mean(2), reg_offset)
...
class PointOccSelfAttention(BaseModule):
    ...
    def calc_points_dists(self, points):
        points = decode_points(points, self.pc_range).mean(dim=2)
...
class PointOccSampling(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super().__init__(init_cfg)
        assert num_refines == num_groups * num_points
        ...
        self.scale_weights = nn.Linear(embed_dims, num_refines * num_levels)
    ...
    def inner_forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        decoded_points = decode_points(query_points, self.pc_range)
        sampling_points = decoded_points.reshape(B, Q, 1, self.num_groups, self.num_points, 3)
