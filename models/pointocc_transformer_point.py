...
class PointOccTransformerPoint(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_points=4,
        ...
        super(PointOccTransformerPoint, self).__init__(init_cfg=init_cfg)
...
class PointOccSampling(BaseModule):
    ...
    def inner_forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        query_std = query_points.std(dim=2, keepdim=True)

        # sampling offset of all frames
        sampling_offset = self.sampling_offset(query_feat)
        sampling_offset = sampling_offset.view(B, Q, -1, 3)
        sampling_offset = sampling_offset / sampling_offset.std(dim=2, keepdim=True) * query_std
