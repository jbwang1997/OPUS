...
class PointOccTransformerCascade(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_points=4,
        ...
        super(PointOccTransformerCascade, self).__init__(init_cfg=init_cfg)
            ...
            for i in range(num_layers):
                self.decoder_layers.append(
                    PointOccTransformerDecoderLayer(
                        ...
                        num_refines[i], layer_idx=i, scale=scales[i], pc_range=pc_range)
                )
        else:
            self.decoder_layers = PointOccTransformerDecoderLayer(
                embed_dims, num_frames, num_points, num_levels, num_classes, 
                num_refines[-1], layer_idx=0, scale=scales[0], pc_range=pc_range
            )
