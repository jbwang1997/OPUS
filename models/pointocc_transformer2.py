...
class PointOccTransformer2(BaseModule):
    def __init__(self,
                 embed_dims,
                 point_dims,
                 num_frames=8,
        ...
        super(PointOccTransformer2, self).__init__(init_cfg=init_cfg)
        ...
        self.decoder = PointOccTransformerDecoder(
            embed_dims, point_dims, num_frames, num_points, num_layers, num_levels,
            num_classes, num_refines, offset_scale, pc_range=pc_range,
...
class PointOccTransformerDecoder(BaseModule):
        ...
        for i in range(num_layers):
            self.decoder_layers.append(
                PointOccTransformerDecoderLayer(
                    embed_dims, point_dims, num_frames, num_points, num_levels,
                    num_classes, num_refines, layer_idx=i, offset_scale=offset_scale,
    ...
    def forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        query_feat = query_feat.new_zeros(
            *query_feat.shape[:2], self.num_refines, self.point_dims)
        query_points = query_points.unsqueeze(2).expand(-1, -1, self.num_refines, -1)
        for i, decoder_layer in enumerate(self.decoder_layers):
            DUMP.stage_count = i

            query_feat, cls_score, refine_pt = decoder_layer(
                query_points, query_feat, mlvl_feats, img_metas)
            query_points = refine_pt.detach()
...
class PointOccTransformerDecoderLayer(BaseModule):
    def __init__(self,
        ...
        self.position_encoder = nn.Sequential(
            nn.Linear(3, self.point_dims), 
            nn.LayerNorm(self.point_dims),
            nn.ReLU(inplace=True),
            nn.Linear(self.point_dims, self.point_dims),
            nn.LayerNorm(self.point_dims),
            nn.ReLU(inplace=True),
        )

        self.self_attn = PointOccSelfAttention(embed_dims, point_dims, num_refines=num_refines,
                                               num_heads=8, dropout=0.1, pc_range=pc_range)
        self.sampling = PointOccSampling(embed_dims, point_dims, num_frames=num_frames, num_groups=4,
                                         num_points=num_points, num_levels=num_levels,
                                         pc_range=pc_range)
        self.mixing = AdaptiveMixing(in_dim=embed_dims, in_points=num_frames, n_groups=4, 
                                     query_dim=self.point_dims, out_points=8)
        self.ffn = FFN(point_dims, feedforward_channels=256, ffn_drop=0.1)

        self.norm1 = nn.LayerNorm(point_dims)
        self.norm2 = nn.LayerNorm(point_dims)
        self.norm3 = nn.LayerNorm(point_dims)

        cls_branch = []
        for _ in range(num_cls_fcs):
            cls_branch.append(nn.Linear(self.point_dims, self.point_dims))
            cls_branch.append(nn.LayerNorm(self.point_dims))
            cls_branch.append(nn.ReLU(inplace=True))
        cls_branch.append(nn.Linear(self.point_dims, self.num_classes))
        self.cls_branch = nn.Sequential(*cls_branch)

        reg_branch = []
        for _ in range(num_reg_fcs):
            reg_branch.append(nn.Linear(self.point_dims, self.point_dims))
            reg_branch.append(nn.ReLU(inplace=True))
        reg_branch.append(nn.Linear(self.point_dims, 3))
    ...
    def refine_points(self, points_proposal, refine_delta):
        points_proposal = decode_points(points_proposal, self.pc_range)
        refine_pt = points_proposal + refine_delta * self.offset_scale
        return encode_points(refine_pt, self.pc_range)
    ...
    def forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        cls_score = self.cls_branch(query_feat)  # [B, Q, R, num_classes]
        reg_offset = self.reg_branch(query_feat)  # [B, Q, R, 3]
...
class PointOccSelfAttention(BaseModule):
    ...
    def __init__(self, 
        ...
        self.in_proj = nn.Sequential(
            nn.Linear(point_dims * num_refines, embed_dims),
        ...
        self.out_proj = nn.Sequential(
            ...
            nn.LayerNorm(embed_dims),
            nn.ReLU(inplace=True),
            nn.Linear(embed_dims, point_dims * num_refines),
        )
        self.attention = MultiheadAttention(embed_dims, num_heads, dropout, batch_first=True)
        self.gen_tau = nn.Linear(embed_dims, num_heads)
    ...
    def inner_forward(self, query_points, query_feat):
        ...
        query_feat: [B, Q, C]
        """
        B, Q, R = query_feat.shape[:3]
        feat = query_feat.flatten(2, 3)
        feat = self.in_proj(feat)

        dist = self.calc_points_dists(query_points)
        tau = self.gen_tau(feat)  # [B, Q, 8]
        ...
        attn_feat = self.attention(feat, attn_mask=attn_mask)

        attn_feat = self.out_proj(attn_feat).reshape(B, Q, R, -1)
        return query_feat + attn_feat
...
class PointOccSampling(BaseModule):
    ...
    def __init__(self,
        ...
        self.sampling_offset = nn.Linear(point_dims, num_groups * 3)
        self.scale_weights = nn.Linear(point_dims, num_groups * num_levels)

    def init_weights(self):
        bias = self.sampling_offset.bias.data.view(self.num_groups, 3)
    ...
    def inner_forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        query_feat: [B, Q, C]
        '''
        B, Q, R = query_points.shape[:3]
        image_h, image_w, _ = img_metas[0]['img_shape'][0]

        # sampling offset of all frames
        sampling_offset = self.sampling_offset(query_feat)
        sampling_offset = sampling_offset.view(B, Q, R, self.num_groups, 3).permute(0, 1, 3, 2, 4)

        xyz = decode_points(query_points, self.pc_range)  # [B, Q, R, 3]
        xyz = xyz[..., None, :, :]  # [B, Q, 1, R, 3]
        sampling_points = xyz + sampling_offset  # [B, Q, G, P, 3]
        sampling_points = sampling_points.reshape(B, Q, 1, self.num_groups, R, 3)
        sampling_points = sampling_points.expand(B, Q, self.num_frames, self.num_groups, R, 3)

        # scale weights
        scale_weights = self.scale_weights(query_feat).view(B, Q, R, self.num_groups, self.num_levels)
        scale_weights = torch.softmax(scale_weights, dim=-1)
        scale_weights = scale_weights.permute(0, 1, 3, 2, 4).unsqueeze(3)
        scale_weights = scale_weights.expand(B, Q, self.num_groups, self.num_frames, R, self.num_levels)
        ...
        sampled_feats = sampled_feats.reshape(B, Q, self.num_groups, self.num_frames, R, -1)
        sampled_feats = sampled_feats.permute(0, 1, 4, 2, 3, 5)  # [B, Q, R, G, T, C]
...
class AdaptiveMixing(nn.Module):
    ...
    def inner_forward(self, x, query):
        ...
        params = params.reshape(B*Q*R, G, -1)
        out = x.reshape(B*Q*R, G, P, C)

        M, S = params.split([self.m_parameters, self.s_parameters], 2)
        M = M.reshape(B*Q*R, G, self.eff_in_dim, self.eff_out_dim)
        S = S.reshape(B*Q*R, G, self.out_points, self.in_points)
