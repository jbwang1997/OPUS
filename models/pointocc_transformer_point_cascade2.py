...
class PointOccTransformerPointCascade2(BaseModule):
    def __init__(self,
                 embed_dims,
                 num_frames=8,
                 num_points=4,
                 ...
                 update_pos=[True, True, True, True, True, True],
                 scales=[8.0],
                 sep_layer=True,
                 pc_range=[],
                 init_cfg=None):
        ...
        super(PointOccTransformerPointCascade2, self).__init__(init_cfg=init_cfg)
        ...
        self.decoder = PointOccTransformerDecoder(
            ...
            num_classes, num_refines, update_pos, scales, sep_layer,
            pc_range=pc_range)
...
class PointOccTransformerDecoder(BaseModule):
            ...
            for i in range(num_layers):
                self.decoder_layers.append(
                    PointOccTransformerDecoderLayer(
                        ...
                        num_refines[i], last_refines[i], update_pos[i], layer_idx=i,
                        scale=scales[i], pc_range=pc_range)
                )
        else:
            self.decoder_layers = PointOccTransformerDecoderLayer(
                embed_dims, num_frames, num_points, num_levels, num_classes, 
                num_refines[-1], last_refines[-1], layer_idx=0, scale=scales[0],
                pc_range=pc_range
            )
    ...
    def forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        query_pos = query_points.mean(dim=2)
        if self.sep_layer:
            for i, decoder_layer in enumerate(self.decoder_layers):
                ...
                query_feat, cls_score, query_points, query_pos = decoder_layer(
                    query_points, query_pos, query_feat, mlvl_feats, img_metas)
            ...
            for i in range(self.num_layers):
                ...
                query_points = query_points.detach()
                query_feat, cls_score, query_points = self.decoder_layers(
                    query_points, query_feat, mlvl_feats, img_metas)
...
class PointOccTransformerDecoderLayer(BaseModule):
    ...
    def forward(self, query_points, query_pos, query_feat, mlvl_feats, img_metas):
        """
        query_points: [B, Q, 3] [x, y, z]
        """
        pos_embed = self.position_encoder(query_points.flatten(2, 3))
        query_feat = query_feat + pos_embed
        ...
        refine_pt = self.refine_points(query_pos, reg_offset)

        if self.update_pos:
            query_pos = refine_pt.mean(dim=2)
        ...
        return query_feat, cls_score, refine_pt, query_pos
...
class PointOccSampling(BaseModule):
    ...
    def __init__(self,
        ...
        # self.sampling_center = nn.Linear(embed_dims, 3)
        # self.sampling_scale = nn.Linear(embed_dims, 3)
        self.scale_weights = nn.Linear(embed_dims, num_groups * num_points * num_levels)

    def init_weights(self):
        bias = self.sampling_offset.bias.data.view(self.num_groups * self.num_points, 3)
        nn.init.zeros_(self.sampling_offset.weight)
        # nn.init.zeros_(self.sampling_center.weight)
        # nn.init.zeros_(self.sampling_scale.weight)
    ...
    def inner_forward(self, query_points, query_feat, mlvl_feats, img_metas):
        ...
        # # mean, std = sampling_offset.mean(2, keepdim=True), sampling_offset.std(2, keepdim=True)
        # # sampling_offset = (sampling_offset - mean) / (std + 1e-4)

        # # sampling_center = self.sampling_center(query_feat).unsqueeze(2) + query_center
        # sampling_center = query_center
        # # sampling_scale = self.sampling_scale(query_feat).unsqueeze(2).exp() * query_scale
        # sampling_scale = query_scale
        # sampling_points = sampling_offset * sampling_scale + sampling_center
